...
fuseserver_setattr(fuse_req_t req, fuse_ino_t ino, struct stat *attr, int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
	...
	if(yfs->isfile(inum)){
		if(yfs->resize(inum, attr->st_size) == yfs_client::OK)
      ...
      off_t off, struct fuse_file_info *fi)
{
  // You fill this in for Lab 2
	...
	if(yfs->isfile(inum))
		if(yfs->getfile(inum, finfo) == yfs_client::OK){
				char *buf = new char[size];
				size_t readb;
				if(yfs->read(inum, buf, size, off, readb) == yfs_client::OK){
					printf("fuse_read done buf:%s size:%d\n", buf, readb);
  ...
  struct fuse_file_info *fi)
{
  // You fill this in for Lab 2
	...
	size_t	written;
	if(yfs->isfile(inum))
		if(yfs->write(inum, buf, size, off, written) == yfs_client::OK)
     ...
     mode_t mode, struct fuse_entry_param *e)
{
  ...
  //
	...
	r = yfs->create(parent, name, new_inum);

	if(r == yfs_client::OK || r == yfs_client::EXIST){
		e->ino = (fuse_ino_t)new_inum;
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  // e.attr appropriately.
	...
	yfs_client::inum findinum;
	if(ret=yfs->lookup(parent, name, findinum) == yfs_client::OK){
		printf("==============================lookup found\n");
		e.ino = (fuse_ino_t)findinum;
          ...
          off_t off, struct fuse_file_info *fi)
{
  ...
  // in directory inum, and call dirbuf_add() for each.
	...
	if(yfs->readir(inum, dirents) == yfs_client::OK)
     ...
     mode_t mode)
{
  ...
  //
	...
	yfs_client::inum new_inum;
	r = yfs->mkadir(parent, name, new_inum);
	printf("in fuse mkdir\n");
	if(r == yfs_client::OK){
		e.ino = (fuse_ino_t)new_inum;
		r = getattr(e.ino, e.attr);
		fuse_reply_entry(req, &e);
...
fuseserver_unlink(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  // Not found:	fuse_reply_err(req, ENOENT);
	...
	if((r=yfs->remove(parent, name)) == yfs_client::OK)
